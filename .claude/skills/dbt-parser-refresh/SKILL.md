...
- **Schema location:** `packages/dbt-artifacts-parser/resources/json-schema/{catalog,manifest,run-results,sources,freshness,semantic_manifest}/` (JSON Schemas for codegen; kept separate from `resources/manifest/` etc. so fixture discovery does not treat schemas as artifacts). Each `*_vN.json` file is turned into `src/<category>/vN.ts`. Legacy `resources/<category>/` is still accepted for catalog/sources when present.
- **Artifact categories:** `catalog`, `manifest`, `run-results`, `sources`, `freshness`, `semantic_manifest`.
