...
- **E2E tests:** `pnpm test:e2e` (runs `pnpm --filter @dbt-tools/web test:e2e`). Install browsers once via `pnpm --filter @dbt-tools/web run test:e2e:install` if Playwright reports a missing executable. CI: [`.github/workflows/test-dbt-tools-web.yml`](.github/workflows/test-dbt-tools-web.yml). Run after meaningful UI or user-flow changes
