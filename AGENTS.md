...
- **PR visual captures (CI):** On pull requests, [`.github/workflows/pr-captures.yml`](.github/workflows/pr-captures.yml) runs in two jobs: **capture** (default `contents: read` only) resolves affected UI areas from [`packages/dbt-tools/web/capture-rules.json`](packages/dbt-tools/web/capture-rules.json), runs [`packages/dbt-tools/web/e2e/pr-capture.spec.ts`](packages/dbt-tools/web/e2e/pr-capture.spec.ts) via [`packages/dbt-tools/web/playwright.capture.config.ts`](packages/dbt-tools/web/playwright.capture.config.ts) (screenshots + annotated WebM demos using fixture data), and uploads artifacts including manifest metadata; **notify** (`pull-requests: write`) downloads that metadata, composes the comment, and posts/updates the PR comment. Extend rules or [`packages/dbt-tools/web/e2e/pr-capture-targets.ts`](packages/dbt-tools/web/e2e/pr-capture-targets.ts) when adding views.
- **PR visual captures (local):** From the repo root, `pnpm capture:resolve` runs [`scripts/pr-capture-resolve.sh`](scripts/pr-capture-resolve.sh) (`git diff` vs `main`, override with `BASE_REF`, e.g. `BASE_REF=origin/main`) and writes `packages/dbt-tools/web/pr-capture-artifacts/capture-manifest.json`. `pnpm capture:record` runs `pnpm --filter @dbt-tools/web capture:record` (build + Playwright capture). `pnpm capture:pr` runs resolve then record (same as [`scripts/run-pr-capture-local.sh`](scripts/run-pr-capture-local.sh)). Output: `packages/dbt-tools/web/pr-capture-artifacts/screenshots/*.png` and `.../videos/*.webm` (gitignored). If you already have a manifest, run only `pnpm capture:record`. To force specific views, write `capture-manifest.json` by hand or set `PR_CAPTURE_MANIFEST` / `PR_CAPTURE_OUTPUT` when invoking Playwright.
