...
- **Trunk git hooks for cloud agents (Codex / Claude Code / Cursor / Copilot coding agents):** After cloning and before edits, run **`pnpm exec trunk git-hooks install`** once per checkout so local commits and pushes run the same Trunk checks as CI (`trunk-fmt-pre-commit`, `trunk-check-pre-push` from [`.trunk/trunk.yaml`](.trunk/trunk.yaml)). In disposable cloud workspaces, repeat this bootstrap whenever the workspace is recreated.
