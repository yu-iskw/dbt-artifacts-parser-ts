...
- **Trunk bootstrap for cloud agents (Codex / Claude Code / Cursor / Copilot coding agents):** After cloning and before edits in ephemeral workspaces, run **`pnpm trunk:bootstrap`** (or the explicit pair: `pnpm exec trunk install && pnpm exec trunk git-hooks install`) once per checkout. This pre-installs Trunk-managed tools for the workspace and installs Git hooks so local commits and pushes run the same Trunk actions as CI (`trunk-fmt-pre-commit`, `trunk-check-pre-push` from [`.trunk/trunk.yaml`](.trunk/trunk.yaml)).
