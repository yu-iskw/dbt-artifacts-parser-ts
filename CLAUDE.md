...
- **Cloud-agent bootstrap:** In ephemeral coding-agent environments, run `pnpm trunk:bootstrap` once per checkout (equivalent to `pnpm exec trunk install && pnpm exec trunk git-hooks install`) so Trunk tool downloads and pre-commit/pre-push actions align with CI (canonical details in [AGENTS.md](AGENTS.md)).
