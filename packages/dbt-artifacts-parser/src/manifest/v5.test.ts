...
import { HttpsSchemasGetdbtComDbtManifestV5Json } from "./v5";

describe("manifest v5", () => {
  it("should parse manifest.json correctly", () => {
    const jsonPath = path.join(
      __dirname,
      "../tests/resources/manifest/v5/jaffle_shop/manifest.json",
    );
    const jsonContent = fs.readFileSync(jsonPath, "utf-8");
    const parsed = JSON.parse(
      jsonContent,
    ) as HttpsSchemasGetdbtComDbtManifestV5Json;
