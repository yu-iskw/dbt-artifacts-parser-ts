...
import { HttpsSchemasGetdbtComDbtManifestV9Json } from "./v9";

describe("manifest v9", () => {
  it("should parse jaffle_shop_at_1.5rc1 manifest.json correctly", () => {
    const jsonPath = path.join(
      __dirname,
      "../tests/resources/manifest/v9/jaffle_shop_at_1.5rc1/manifest.json",
    );
    const jsonContent = fs.readFileSync(jsonPath, "utf-8");
    const parsed = JSON.parse(
      jsonContent,
    ) as HttpsSchemasGetdbtComDbtManifestV9Json;
