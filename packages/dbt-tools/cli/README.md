...
graph TD
  ...
  CLI --> freshnessRpt["freshness-report\nsource freshness"]
  CLI --> staleImpact["stale-impact\ndownstream impact"]
  ...
  freshnessRpt -->|sources.json\n+ manifest.json| SFA[SourceFreshnessAnalyzer]
  staleImpact -->|manifest.json| MG
  staleImpact -->|sources.json| SFA
...
- **Source freshness**: Analyze data freshness of dbt sources with `freshness-report`
- **Stale impact analysis**: Compute downstream impact of stale sources with `stale-impact`
...
Report normalized source freshness results from `sources.json`. Shows which sources are stale, warning, erroring, or healthy.

Requires `sources.json` (generated by `dbt source freshness`). Optionally enriches results with manifest metadata.

```bash
# Uses ./target/sources.json by default
dbt-tools freshness-report

# Show only stale sources
dbt-tools freshness-report --stale-only

# Filter by status
dbt-tools freshness-report --status warn,error

# With manifest enrichment (adds tags, description, path info)
dbt-tools freshness-report path/to/sources.json path/to/manifest.json

# Field filtering
dbt-tools freshness-report --fields "unique_id,name,status,max_loaded_at,age_seconds"

# JSON output
dbt-tools freshness-report --json
```

**Options:**

- `[sources-path]` - Path to sources.json (defaults to `./target/sources.json`)
- `[manifest-path]` - Path to manifest.json (optional, for enrichment with tags/description/path)
- `--target-dir <dir>` - Custom target directory
- `--status <values>` - Comma-separated filter by freshness status (e.g., `pass,warn,error`)
- `--stale-only` - Filter to stale sources only (warn, error, runtime error)
- `--fields <fields>` - Comma-separated list of fields to include
- `--json` - Force JSON output
- `--no-json` - Force human-readable output
...
Analyze downstream impact of a stale source in the dependency graph. Shows how many models, tests, and exposures depend on a source that has stopped loading fresh data.

Requires `manifest.json`. Optionally enriches results with freshness status from `sources.json`.

```bash
# Show downstream impact of a source
dbt-tools stale-impact source.my_project.raw.orders

# Limit impact depth to 2 hops
dbt-tools stale-impact source.my_project.raw.orders --depth 2

# With freshness enrichment
dbt-tools stale-impact source.my_project.raw.orders --sources-path ./target/sources.json

# Custom paths
dbt-tools stale-impact source.my_project.raw.orders \
  --manifest-path ./custom/manifest.json \
  --sources-path ./custom/sources.json

# Field filtering
dbt-tools stale-impact source.my_project.raw.orders \
  --fields "source,direct_downstream_count,total_downstream_count,impacted_counts_by_type"

# JSON output
dbt-tools stale-impact source.my_project.raw.orders --json
```

**Options:**

- `<source-unique-id>` - Source unique_id (e.g., `source.my_project.raw.orders`)
- `--manifest-path <path>` - Path to manifest.json (defaults to `./target/manifest.json`)
- `--sources-path <path>` - Path to sources.json (optional, for freshness enrichment)
- `--target-dir <dir>` - Custom target directory
- `--depth <n>` - Max traversal depth (limits downstream impact search)
- `--fields <fields>` - Comma-separated list of fields to include
- `--json` - Force JSON output
- `--no-json` - Force human-readable output
...
```json
{
  ...
  "direct_downstream_count": 3,
  "total_downstream_count": 18,
