 ...
 * gantt rows (after default timeline scoping). Helps explain missing types such as
 * `source` when dbt did not execute them but the manifest defines them.
 */
export function buildInvocationResourceComparison(
  analysis: AnalysisState,
  projectName: string | null,
): InvocationResourceComparisonRow[] {
  const graphByType = analysis.graphSummary.nodesByType;
