# Repository agent plugins (Codex, Claude Code, and Cursor)

This monorepo ships **first-party agent plugins** under `plugins/<plugin-id>/`. Each plugin uses a shared layout: `.claude-plugin/plugin.json`, `.codex-plugin/plugin.json`, `.cursor-plugin/plugin.json`, and `skills/` at the plugin root (see upstream docs for [Claude Code](https://code.claude.com/docs/en/plugins), [Codex](https://developers.openai.com/codex/plugins/build), and the [cursor/plugins](https://github.com/cursor/plugins) reference repository for Cursor layout and manifests).

## Codex marketplace (single catalog)

**Codex** discovers one repo-scoped marketplace file:

- [`.agents/plugins/marketplace.json`](../.agents/plugins/marketplace.json)

Add a new plugin by creating `plugins/<plugin-id>/` and appending an entry to the `plugins` array there (`source.source: "local"`, `source.path` like `./plugins/<plugin-id>`, plus `policy` and `category` per Codex).

## Claude Code

Per [Claude Code plugin marketplaces](https://code.claude.com/docs/en/plugin-marketplaces), a **hosted** marketplace is a `marketplace.json` catalog—typically **`.claude-plugin/marketplace.json` at the root of a marketplace repository**—that lists plugins and their sources.

**This monorepo does not commit that file.** Catalog JSON for agents in this project lives elsewhere on purpose:
...
**Claude Code** uses each plugin’s on-disk layout under `plugins/<plugin-id>/`, especially [`plugins/dbt-tools-cli/.claude-plugin/plugin.json`](dbt-tools-cli/.claude-plugin/plugin.json) as an example, plus project or user configuration (e.g. [`.claude/settings.json`](../.claude/settings.json)). Those folders are the same plugin artifacts the Codex marketplace points at; they are not duplicated into a Claude-hosted `marketplace.json` here.

To register a marketplace the way Claude’s docs describe (for example `/plugin marketplace add` with a path or URL), see Claude’s [Discover and install plugins](https://code.claude.com/en/discover-plugins) and [plugin marketplaces](https://code.claude.com/docs/en/plugin-marketplaces) guides.

## Cursor

**Cursor** uses a repo-scoped marketplace at [`.cursor-plugin/marketplace.json`](../.cursor-plugin/marketplace.json) and per-plugin [`.cursor-plugin/plugin.json`](../plugins/dbt-tools-cli/.cursor-plugin/plugin.json) under each `plugins/<plugin-id>/`. Keep the Cursor marketplace **in sync** with the Codex catalog (same plugin ids and `./plugins/<plugin-id>` sources); verification enforces this when `PLUGIN_FILTER` is unset.

## Index

| Plugin id     | Path                    | Purpose                                                      |
| ------------- | ----------------------- | ------------------------------------------------------------ |
| dbt-tools-cli | `plugins/dbt-tools-cli` | Minimal shared plugin; extend for `@dbt-tools/cli` workflows |
